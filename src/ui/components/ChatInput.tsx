...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  return (
    <div className="flex flex-col h-full max-h-[620px] select-none">
      ...
      {/* 6. Solid Elevated Bottom Toolbar (Two-Row Layout for Clear Hierarchy & Zero Clipping) */}
      <div className="relative z-30 px-3.5 py-2.5 bg-[#131320] border-t border-white/[0.08] flex flex-col gap-2 shrink-0">
        {/* Row 1: Model Selector (Left) + Quick Repair Pill (Right) */}
        <div className="flex items-center justify-between gap-2">
          ...
          {onAutoRepair && (
            <button
              ...
              title="1-Click Fix LaTeX macro errors and restore preamble"
              className="px-2.5 py-1 text-[11px] font-medium rounded-lg bg-[#18182a] hover:bg-amber-500/20 text-amber-300 hover:text-amber-200 border border-white/[0.08] hover:border-amber-500/40 transition-colors flex items-center gap-1.5 active:scale-[0.98]"
            >
              <Wand2 className="w-3 h-3 text-amber-400" />
              <span>Fix LaTeX</span>
            </button>
        ...
        {/* Row 2: Full-Width Primary Action (Generate / Stop) with Ample Breathing Room */}
        {isGenerating ? (
          <button
            type="button"
            onClick={onStop}
            className="w-full flex items-center justify-center gap-2 py-2 px-4 rounded-xl text-xs font-semibold text-rose-200 bg-rose-500/30 hover:bg-rose-500/40 border border-rose-500/40 transition-all active:scale-[0.99] animate-pulse shadow-md shadow-rose-950/40"
          >
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2 px-4 rounded-xl text-xs font-semibold text-white bg-gradient-to-r from-violet-600 via-indigo-600 to-blue-600 hover:from-violet-500 hover:to-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed border border-indigo-400/30 shadow-md shadow-indigo-600/30 hover:shadow-indigo-600/50 transition-all active:scale-[0.99]"
          >
            <Sparkles className="w-3.5 h-3.5" />
            <span>Generate</span>
            <KeyboardShortcutHint
              shortcut="Ctrl+↵"
              variant="on-accent"
              className="inline-flex ml-1 opacity-90"
            />
          </button>
        )}
      </div>
